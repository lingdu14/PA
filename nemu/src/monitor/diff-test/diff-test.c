...
void difftest_step(uint32_t eip) {
    ...
    // 对比 QEMU 返回的寄存器状态 (r) 和 NEMU 当前的寄存器状态 (cpu)
    if (r.eax != cpu.eax || r.ecx != cpu.ecx || r.edx != cpu.edx ||
        ...
        diff = true; // 发现不一致，触发 diff 错误

        // 打印错误信息
        printf("Difftest failed at EIP = 0x%08x\n", cpu.eip);
        printf("QEMU: EAX=0x%08x, ECX=0x%08x, EDX=0x%08x, EBX=0x%08x\n", r.eax, r.ecx, r.edx, r.ebx);
        printf("NEMU: EAX=0x%08x, ECX=0x%08x, EDX=0x%08x, EBX=0x%08x\n", cpu.eax, cpu.ecx, cpu.edx, cpu.ebx);
